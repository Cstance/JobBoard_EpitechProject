...
function applyRedirection(id){
    const redirectURL = `http://localhost:5173/Apply?id=${id}`;
...
<section id="{ad.id}">
    ...
    <a href="http://localhost:5173/Apply?job">
	<button on:click={()=> applyRedirection(ad.id)} class="button" type="submit"> Apply </button>
    </a>
    </section>
    {/each}
    {:catch error}
